 ...
 * @LastEditTime: 2020-09-15 09:43:36
    ...
    recordData2: {
      handler(newValue, oldValue) {
        ...
        if (newValue.distanceDataArray.length !== 0) {
          ...
          if (minDistance < 0) {
            for (
              let i = 0, len = newValue.distanceDataArray.length;
              i < len;
              i++
            ) {
        ...
        /* x轴 */
        const xAxisData = []
        for (let i = 0, len = newValue.distanceDataArray.length; i < len; i++) {
        ...
        this.myChartMaxPower.setOption({
          ...
          toolbox: {
            ...
            feature: {
              // 导出图片
              saveAsImage: {
                name: `各段峰值功率图_${this.$moment().format('YYYY-MM-DD')}`
              },
              restore: {} // 重置
            }
          },
